...
# this code uses the Base class together with the engine connection variable to
# 1- drop all existing tables
# 2- create any tables that Base mapped, in a class that inherits Base (like User)


# any time you want to perform a CRUD operation w/ SQLAlchemy you need to establish a temporary session with the Session class 
db = Session()

# insert users
# this alone does not change the db - it just preps the INSERT statements
db.add_all([
...
# you need to run the INSERT statements by calling db.commit()
db.commit()

# insert posts
# must be run after creating users because the post model requires valid user_id fields
db.add_all([
...
# need to run db.commit() again to confirm these latest insertions
db.commit()
